...
async function respaldoGroqBatch(titulos) {
    ...
    try {
        const chatCompletion = await groq.chat.completions.create({
            messages: [
                { role: "system", content: "Eres un API que devuelve EXCLUSIVAMENTE JSON arrays. No uses markdown. No des explicaciones." },
...
// IA PRINCIPAL EN LOTES (GEMINI) - ¡CORREGIDA!
async function generarMarketingIABatch(titulos) {
    try {
        const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
        const prompt = `Analiza esta lista de productos y genera un JSON estructurado para cada uno con frase persuasiva corta y hashtags:
        ${titulos.map((t, i) => `${i + 1}. ${t}`).join('\n')}
        Devuelve estrictamente un arreglo con este formato JSON por cada elemento: [{"seguro_para_fb": true, "frase": "...", "hashtags": "#Tag"}]. No uses markdown, entrega solo el JSON crudo.`;
        ...
        return titulos.map(() => ({ seguro_para_fb: true, frase: "¡Oferta increíble! ⚡", hashtags: "#Ofertas" }));
    }
}
...
async function respaldoGroqIndividual(titulo) {
    ...
    try {
        const chatCompletion = await groq.chat.completions.create({
            messages: [
                { role: "system", content: "You output pure JSON objects only." },
...
async function generarMarketingIA(titulo) {
    try {
        const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
        const prompt = `Analiza: "${titulo}". Genera un JSON EXACTO: {"seguro_para_fb": true, "frase": "frase persuasiva corta", "hashtags": "#Tag1 #Tag2"}. Usa siempre true para seguro_para_fb. Solo JSON sin markdown.`;
        const result = await model.generateContent(prompt);
        let jsonString = result.response.text().replace(/```(json)?/gi, '').trim();
        return JSON.parse(jsonString);
    } catch (e) {
