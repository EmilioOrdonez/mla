...
// --- FUNCIÓN ACORTADORA (Para uso manual y automático) ---
async function acortarLink(urlLarga) {
    try {
        ...
        console.error("⚠️ Error TinyURL:", e.message);
        return urlLarga;
    }
}
...
<head>
    ...
    <style>
        ...
        .container { width: 100%; max-width: 600px; background: #16213e; padding: 25px; border-radius: 24px; box-shadow: 0 15px 35px rgba(0,0,0,0.6); border: 1px solid #0f3460; text-align: center; }
        h1 { color: var(--primary); margin: 0; font-weight: 700; letter-spacing: 1px; font-size: 1.6rem; }
        .dashboard { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; margin: 25px 0; }
        .stat-box { background: #0f3460; padding: 15px 5px; border-radius: 15px; border: 1px solid #1a1a2e; }
        .stat-num { font-size: 1.5rem; font-weight: bold; }
        .stat-label { font-size: 0.6rem; text-transform: uppercase; color: #888; margin-top: 5px; }
        textarea { width: 100%; background: #0f3460; border: 1px solid #1a1a2e; border-radius: 15px; color: #fff; padding: 15px; box-sizing: border-box; resize: none; margin-bottom: 15px; font-size: 0.9rem; }
        .btn { display: inline-flex; align-items: center; justify-content: center; padding: 15px 25px; border-radius: 50px; text-decoration: none; font-weight: bold; transition: 0.3s; cursor: pointer; border: none; font-size: 0.9rem; width: 100%; box-sizing: border-box; }
        .btn-primary { background: linear-gradient(45deg, var(--primary), var(--secondary)); color: #fff; margin-top: 5px; }
        .btn-back { background: #0f3460; color: #aaa; margin-top: 15px; border: 1px solid #1a1a2e; }
        .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-top: 20px; }
        .card { background: #1a1a2e; padding: 15px; border-radius: 15px; text-align: left; border-left: 4px solid var(--primary); margin-bottom: 10px; font-size: 0.85rem; }
        .spinner { display: none; width: 18px; height: 18px; border: 3px solid rgba(255,255,255,0.3); border-radius: 50%; border-top-color: #fff; animation: spin 1s infinite; margin-right: 10px; }
